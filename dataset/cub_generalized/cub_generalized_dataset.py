...
from utils import join, load, read_image, file_cached, mask2bbox, mkdir


class CubGeneralizedDataset(Dataset):
    ...
    def augmented_root(self):
        # The augmented root for this dataset is different
        root_head, root_tail = os.path.split(self.root)
        root_tail = "CUB_GENERALIZED"
    ...
    def _build_concept_split_specs(self):
        # All concepts from classes are train concepts because those could be directly grounded via image
        #
        ...
        train_concepts = set(self.classes)
        higher = [x for x in self.concepts if x not in self.classes]
        train_concepts.update([_ for _ in higher if _ % 4 in train_remainder])
        val_concepts = set(_ for _ in higher if _ % 4 in val_remainder)
        test_concepts = set(_ for _ in higher if _ % 4 in test_remainder)
        split_spec = [0 for _ in range(len(self.concepts))]
        for concept in train_concepts:
            split_spec[concept] = 0
        for concept in val_concepts:
            split_spec[concept] = 1
        for concept in test_concepts:
            split_spec[concept] = 2
        return split_spec
    ...
    def metaconcept_text(self, supports, relations, concept_index):
        # For the first stage, all supports should just be hyponyms of the current concepts
        names = list(self.names[s] for s in supports)
        return f"{', '.join(names)} are a kind of " \
               f"{self.names[concept_index]}.".capitalize()
...
class CubGeneralizedBuilderDataset(BuilderDataset, CubGeneralizedDataset):
