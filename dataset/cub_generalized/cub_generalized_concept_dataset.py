...
class CubGeneralizedConceptDataset(CubGeneralizedDataset, PretrainDataset):
...
class CubGeneralizedConceptSupportDataset(CubGeneralizedConceptDataset):
...
class CubGeneralizedPretrainBuilderDataset(PretrainBuilderDataset, CubGeneralizedBuilderDataset):
    N_SAMPLES = 100
    ...
    def _build_image_questions(self, objects, image_index):
        questions = []
        candidates = [x for x in objects[0] if x in self.concept_sets]
...
class CubGeneralizedConceptSupportBuilderDataset(CubGeneralizedPretrainBuilderDataset):
