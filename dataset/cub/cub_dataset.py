...
class CubDataset(Dataset):
    ...
    def __init__(self, cfg, args):
        ...
        self.concept2splits = (self.concept_split_specs - 1).float().div(self._hierarchy_n).clamp(min=-2.,
            max=2.).floor() + 1
    ...
    def _build_concept_split_specs(self):
        ...
        test_remainder = self.split_seed % 10
        temp_split_range.remove(test_remainder)
        random.shuffle(temp_split_range)
        if self.split_seed in list(range(5)):
            split_remainder = [temp_split_range[:2], temp_split_range[2:], [test_remainder]]
        elif self.split_seed in list(range(10,15)):
            split_remainder = [temp_split_range[:1], temp_split_range[1:], [test_remainder]]
        else:
            split_remainder = [[], temp_split_range, [test_remainder]]
        pretrain_remainder, train_remainder, val_remainder = split_remainder
        pretrain_classes = set(_ for _ in self.classes if _ % 5 in pretrain_remainder)
        train_classes = set(_ for _ in self.classes if _ % 5 in train_remainder)
        val_classes = set(_ for _ in self.classes if _ % 5 in val_remainder)
        pretrain_concepts, train_concepts, val_concepts = set(pretrain_classes), set(train_classes), set(val_classes)
        for pretrain_class in pretrain_classes:
            pretrain_concepts.update(self.hypo2hyper[pretrain_class])
        ...
        #for c in sorted(train_concepts, reverse=True):
            # Guarantee that all antecedent of train concepts are in pretrained
        #    if min(self.hypo2hyper[c]) in pretrain_concepts:
        #        continue
        #    train_concepts.difference_update([min(self.hypo2hyper[c])])
        #    pretrain_concepts.update([min(self.hypo2hyper[c])])
        #for c in sorted(val_concepts, reverse=True):
        #    if min(self.hypo2hyper[c]) in pretrain_val_concepts or min(self.hypo2hyper[c]) in pretrain_concepts:
        #        pretrain_val_concepts.add(c)
        #train_val_concepts = val_concepts.difference(pretrain_val_concepts)
        #for c in sorted(test_concepts, reverse=True):
        #    if min(self.hypo2hyper[c]) in pretrain_test_concepts or min(self.hypo2hyper[c]) in pretrain_concepts:
        #        pretrain_test_concepts.add(c)
        #train_val_test_concepts = test_concepts.difference(pretrain_test_concepts)
        ...
        fill_spec_tree_(split_spec, pretrain_concepts, -9)
        fill_spec_tree_(split_spec, train_concepts, -4)
        fill_spec_tree_(split_spec, val_concepts, 1)
        # We validation and test on all concepts
        #fill_spec_tree_(split_spec, train_val_concepts, 11)
        #fill_spec_tree_(split_spec, train_val_test_concepts, 11)
        return split_spec
    ...
    def _build_word_tokens(self):
        ...
        vocabulary.update(["parent", "great", "grandparent", "of"])
...
class CubBuilderDataset(BuilderDataset, CubDataset):
    ...
    metaconcepts_ = ['parent', 'grandparent', 'great_grandparent', 'great_great_grandparent']
