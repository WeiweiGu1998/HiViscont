...
class PretrainBuilderDataset(BuilderDataset):
    ...
    def batch_inference_handler(self, inputs, outputs):
        ...
        for i, p in enumerate(outputs['question_predicted']):
            ...
            #question = {k: inputs[i][k] for k in ['question', 'answer', 'image_index']}
            # Temporary change
            question = {k: inputs[i][k] for k in ['question', 'answer', 'image_index', 'questioned_concept']}
            questions.append({**question,'program': program, 'family': family, 'length': length})
        return {"questions": questions}

    def save_inference_handler(self,  evaluated, output_dir="/home/local/ASUAD/weiweigu/Desktop"):
        questions = []
        for (image_index, family), qs in groupby(evaluated['questions'],
            ...
            # Temporary change
            questioned_concepts = [q['questioned_concept'] for q in qs]
            ...
            q = {'text': question, 'program': program, 'answer': answer, 'image_index': image_index,
                'length': length, 'questioned_concept': questioned_concepts}
            questions.append(q)
    ...
    def dump_questions(self):
        ...
        for q in self.questions:
            outputs["question_predicted"].append(torch.tensor(q['question_target']))
        questions = self.batch_inference_handler(self.questions, outputs)
        self.save_inference_handler(evaluated=questions)
