...
class Dataset(torch.utils.data.Dataset):
    ...
    def batch_evaluate(self, inputs, outputs, evaluated):
        ...
        # Weiwei's temporary change
        handler = getattr(self, f"batch_{evaluated['mode']}_handler", self.batch_default_handler)
        #handler = getattr(self, f"batch_pretrain_handler", self.batch_default_handler)
    ...
    def evaluate_metric(self, evaluated):
        ...
        #handler = getattr(self, f"metric_pretrain_handler", self.metric_default_handler)
    ...
    def clevr2nscl(clevr_program):
        ...
        for block_id, block in enumerate(clevr_program):
            ...
            elif op == 'is_instance_of':
                concept = block['value_inputs']
                current = dict(op='is_instance_of', concept=concept)
    ...
    def nscl2program(self, nscl_program):
        ...
        for np in nscl_program:
            ...
            if op == "filter":
                ...
                p = ('IsInstanceOf', [self.entry2idx_[c] for c in np['concept']])
            elif op == "query":
...
class BuilderDataset(Dataset):
    MAC_ROOT = "/home/local/ASUAD/weiweigu/Desktop"
    texts = ["statement"]
    num_inputs = {}
    num_value_inputs = {}
