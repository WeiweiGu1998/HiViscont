...
class IsInstanceOf(SymbolicProgram):
    # (TODO): @Weiwei add this program
    # This program is only used to add abstract concepts like colors or affordances
    # input: concept_1, concept_2
    # output: is concept_1 an instance of concept_2?
    # e.g. concept_1: red, concept_2: color :->true
    # e.g. concept_1: red, concept_2: affordance :-> false
    ...
    def __call__(self, executor):
        logits = executor.entailment(
            self.concepts[0,:].unsqueeze(0), self.concepts[1,:].unsqueeze(0)
        )
        query_object = torch.tensor(0, device = logits.device)
        return {
            "end": logits, "queried_embedding": self.concepts, 'query_object': query_object
        }
