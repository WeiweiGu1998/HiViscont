...
from .fewshot_hierarchy import FewshotHierarchy
...
class Composite(SymbolicProgram):
    ...
    def evaluate(self, box_registry, **kwargs):
        k = {}
        # Construct the symbolic programs from dictionaries
        ...
        # Calls SymbolicProgram.evaluate()
        p = super(Composite, self).evaluate(box_registry, **kwargs)
        # Construct the composite programs for actual computation
        for group in ["train_sample", "val_sample"]:
    ...
    def to_fewshot_hierarchy(self):
        fewshot_hierarchy = FewshotHierarchy(self.gt_embeddings, self.relations, self.concept_index)
        return fewshot_hierarchy
