...
class ConceptGraphGNN(nn.Module):
    ...
    def forward(self, embeddings, relations, coefficient=1.):
        ...
        if self.representation == "box":
            # Changes done by Weiwei
            # compute the boundary values of boxes
            # shape: (FxD)
            center, offset = x.chunk(2, -1)
            # shape: F*2D
            box_boundaries = torch.cat([center + offset, center - offset], -1)
            # This operation is just to guarantee that the resulted embedding matches the ones they claim in the paper
            normalized = torch.sigmoid(box_boundaries) - 0.5
            maxima, minima = normalized.chunk(2, -1)
            center = (maxima + minima) / 2
            offset = (maxima - minima) / 2
            x = torch.cat([center, offset], 2)
        return x
