...
class FewshotHierarchyProgramExecutor(nn.Module):
    NETWORK_REGISTRY = {}
    ...
    def __init__(self, cfg):
        ...
        network = self.NETWORK_REGISTRY[cfg.NAME](cfg)
        entailment = build_entailment(cfg)
        self.learner = PipelineHierarchyLearner(network, entailment)
...
class MetaLearner(nn.Module):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        FewshotHierarchyProgramExecutor.NETWORK_REGISTRY[name] = cls
        cls.name = name
...
class PipelineHierarchyLearner(nn.Module):
    def __init__(self, network, entailment):
        super().__init__()
        self.network = network
        self.entailment = entailment
    ...
    def forward(self, p):
        ...
        for q in p.train_program:
            ...
            index = end.squeeze(0).max(0).indices
            shots.append(q.object_collections[index])
        #Shape: (num_shots, 2*box_dimension)
        shots = torch.stack(shots)
        # computes the updated embeddings
        # returns the ouput_dict
        return p.fewshot(shots, self)
