...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.freeze = feature_cfg.FREEZE
        if self.from_feature_dim:
            ...
            # Weiwei's change
            # Untrack the running stats to guarantee the weights for models
            #self.backbone.apply(untrack_running_stats_for_batchnorm)
...
class BatchedFeatureExtractor(FeatureExtractor):
    ...
    def forward(self, xs):
        # This change keeps the behavior of the batched feature extractor consistent with the feature extractor
        if self.freeze:
            self.eval()
        xs = self.forward_backbone(torch.cat([_ for _ in xs])).split([len(_) for _ in xs])
        xs = zip(*(self.forward_feature_relation(x) for x in xs))
        return xs
