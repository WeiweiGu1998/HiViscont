...
class DatasetCatalog:
    ...
    CUB_GENERALIZED_ROOT = join(DATASET_ROOT, "CUB_GENERALIZED")
    GQA_ROOT = join(DATASET_ROOT, "GQA")
    TABLETOP_ROOT = join(DATASET_ROOT, "test_dataset")

    NAME2ROOT = {"clevr": CLEVR_ROOT, "cub": CUB_ROOT, 'gqa': GQA_ROOT, 'tabletop': TABLETOP_ROOT}
    ...
    def get(self, name, domain, args, as_tuple=False):
        if as_tuple:
            names = name.split("&")
            return tuple((n, self.get(n, domain,args)) for n in names)
        if "&" in name:
            names = name.split("&")
        ...
        return build_dataset(
            CN(dict(NAME=dataset_name, DOMAIN=domain, SPLIT=split, ROOT=root, OPTS=parts[split_index + 1:], **self.cfg)), args)
