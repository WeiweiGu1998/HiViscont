...
from dataset import ExtractionDataset
from models import ConceptExtractionModel
...
def parse_args():
    ...
    parser.add_argument('-o', '--output_dir', default='extraction_model', help='output directory')
...
def train(cfg):
    ...
    train_set = ExtractionDataset(cfg.DS)
    val_ds = copy.deepcopy(cfg.DS)
    val_ds.split = "val"
    val_set = ExtractionDataset(val_ds)
    train_loader = DataLoader(train_set, cfg)

    model = ConceptExtractionModel(cfg.M).to("cuda")
    ...
    for i in range(num_iter):
        ...
        progress = tqdm.tqdm(
            total=7000, ncols = 100, desc="Train:"
        )
        for inputs in train_loader:
            ...
            train_predictions.append(outputs["prediction"].unsqueeze(0))
            loss = outputs["loss"]
        ...
        with torch.no_grad():
            model.eval()
            progress = tqdm.tqdm(
                total=3000, ncols = 100, desc="Validation:"
            )
            ...
            for inputs in val_loader:
                ...
                val_predictions.append(outputs["prediction"].unsqueeze(0))
